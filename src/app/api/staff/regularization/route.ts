...
export async function GET(
  ...
  try {
    ...
    const rows =
      await prisma.attendanceRegularization.findMany({
        where: {
          userId,
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "User, Staff, attendance date and reason are required.",
        },
        {
          status: 400,
        }
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unable to create regularization request.",
      },
      {
        status: 500,
      }
...
export async function PATCH(
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Valid request, user and action are required.",
        },
        {
          status: 400,
        }
    ...
    const existing =
      await prisma.attendanceRegularization.findFirst({
        where: {
          id,
          userId,
        },
      ...
      await prisma.$transaction(
        async (
          ...
          if (
            ...
            const attendance =
              await tx.staffAttendance.findFirst({
                where: {
                  staffId:
                    existing.staffId,
            ...
            const checkIn =
              existing.requestedCheckIn ||
              attendance?.checkIn ||
              null;

            const checkOut =
              existing.requestedCheckOut ||
              attendance?.checkOut ||
              null;
